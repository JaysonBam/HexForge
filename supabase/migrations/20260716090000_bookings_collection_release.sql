-- Give the bookings collection desk a strict, atomic release operation without
-- changing HexForge's existing project or part transition behavior.
...
begin
  ...
  if v_project.state <> 'IN_PRODUCTION' then
    return jsonb_build_object(
      'ok', false,
      'errors', jsonb_build_array(format('Only in-production projects can be released from bookings (current: %s).', v_project.state))
    );
  end if;
  ...
  if v_completed_parts <> v_total_parts then
    return jsonb_build_object(
      'ok', false,
      'errors', jsonb_build_array(format('%s of %s parts are complete. Finish every part before moving the project to the help desk.', v_completed_parts, v_total_parts))
    );
  end if;
